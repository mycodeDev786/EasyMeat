...
export default function RegisterPage() {
  ...
  const handleGoogle = async () => {
    try { await loginWithGoogle(); toast.success('Account created!'); router.push('/dashboard'); }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-20" style={{background:'#0A0A0A'}}>
      <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} className="w-full max-w-md">
        ...
        <div className="glass border border-white/10 rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[{name:'name',label:'Full Name',type:'text',ph:'Ahmed Al Mansouri',Icon:FiUser},
              {name:'email',label:'Email Address',type:'email',ph:'you@example.com',Icon:FiMail},
              {name:'phone',label:'Phone Number',type:'tel',ph:'+971 50 000 0000',Icon:FiPhone}].map(({name,label,type,ph,Icon})=>(
              <div key={name}>
                <label className="text-gray-400 text-sm mb-1 block">{label}</label>
                <div className="relative">
                  <Icon className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={16}/>
                  <input name={name} type={type} value={form[name as keyof typeof form]} onChange={handleChange} required placeholder={ph} className={`${inputCls} pl-11 pr-4`}/>
                </div>
              </div>
            ))}
            {[{name:'password',label:'Password',ph:'Min. 6 characters'},{name:'confirm',label:'Confirm Password',ph:'Repeat your password'}].map(({name,label,ph})=>(
              <div key={name}>
                <label className="text-gray-400 text-sm mb-1 block">{label}</label>
                <div className="relative">
                  <FiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={16}/>
                  <input name={name} type={showPwd?'text':'password'} value={form[name as keyof typeof form]} onChange={handleChange} required placeholder={ph} className={`${inputCls} pl-11 pr-12`}/>
                  {name==='password' && <button type="button" onClick={()=>setShowPwd(!showPwd)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white">{showPwd?<FiEyeOff size={16}/>:<FiEye size={16}/>}</button>}
                </div>
              </div>
            ))}
          ...
          <p className="text-center text-gray-400 text-sm mt-6">
            Already have an account? <Link href="/auth/login" className="text-yellow-400 hover:underline font-medium">Sign In</Link>
          </p>
        </div>
      </motion.div>
    </div>
